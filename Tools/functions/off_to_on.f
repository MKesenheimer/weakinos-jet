...
c############### subroutine check_on_shell #############################
      subroutine check_on_shell(pi_os,pj_os,mij)
        implicit none
        double precision pi_os(0:3),pj_os(0:3),mij
        double precision relerror
        double precision momsq, momsum2sq, momsum3sq, dotp
        external momsq, momsum2sq, momsum3sq, dotp
        relerror = (momsum2sq(pi_OS(:),pj_OS(:))-mij**2)
     &             /(momsum2sq(pi_OS(:),pj_OS(:))+mij**2) 
        if( dabs(relerror) .gt. 1D-6 ) then
          print*,"error: no on-shell condition was found."
          print*,"|pi_OS+pj_OS| = ",dsqrt(momsum2sq(pi_OS(:),pj_OS(:)))
          print*,"mij           = ", mij
...
c rescaled by this correction factor
      double precision function corrfac(shat,mi,mj,mk,ml,mij,mkl,sij,skl)
        ...
        corrfac = (sij*skl*kaellenSqrt(shat,mij**2,mkl**2)
