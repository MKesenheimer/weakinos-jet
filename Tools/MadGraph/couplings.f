...
c
      ...
      parameter( Rt2   = 1.414213562373095048801d0 )
...
c
      ...
      real*8    width(0:99)
      save unimass,lowmass,bw,uu,vv, ! bw: neutralino mixing matrix. uu, vv: Chargino mixing matrix 
     $     m_t,m_b,m_l,width
...
c
     ...
     &        bw,uu,vv,m_t,m_b,m_l,param_name)
      endif ! kesenheimer: commented endif from line 140 out.
            ! Read only from slha file if readlha = true
            ! but do the other stuff anyway.
            ! This is important when the parameters given by powheg
            ! need to overwrite the parameters read from slha file.
            ! The local parameters unimass, lowmass... are saved, so it
            ! is no problem to call the subroutine INIT_SUSY (which needs 
            ! these paramters) once again, without reading from slha
            ! file. Note: When calling setpara for the
            ! first time, it is necessary to set readlha = true.
      
      G = DSQRT(Fourpi*ALPHAS) ! use setting of the param_card.dat @ NLO
c
c------------------------------------------
...
c
c kesenheimer:
c set the mixing matrices (for subroutine printMadgraphModelParameters)
c
      bwmix = bw
      uumix = uu
      vvmix = vv
...
!      kesenheimer:
!      madgraph uses wmass (read from slha file) for every other calculation
!      so it is appropriate to use it here as well
!      wm = sqrt(zmass**2/Two+
...
c -- yl given at the scale Q                                     --- c
      if(gw.ne.0)then
        ! kesenheimer: set bottom mass equal to bmass (same for top)
        ! to agree with FormCalc Born amplitude (-> no running)
        ! rmt   = dsqrt(2d0)*wmass*sb*yt/gw ! (old)
        ! rmb   = dsqrt(2d0)*wmass*cb*yb/gw ! (old)
        rmt   = 0
        rmb   = 0
...
#ifdef DEBUGQ
      print*, "[DEBUG] in couplings.f"
      print*, "[DEBUG] gbln1p = ", gbln1p
      print*, "[DEBUG] gbln1m = ", gbln1m
      print*, "[DEBUG] gb1n1p = ", gb1n1p
      print*, "[DEBUG] gb1n1m = ", gb1n1m
#endif
