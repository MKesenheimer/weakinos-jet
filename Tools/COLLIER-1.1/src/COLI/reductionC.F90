...
!  CalcCuv, CalcCpv, CalcCpv2, CalcCg, CalcCgy, CalcCgp, CopyCimp
!
!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
...
module globalC
  ...
  double complex :: q1q2,detZ,Z(2,2),Zadj(2,2),f(2),Zadjf(2),Xadj(0:2,0:2),detX
  ...
  double precision :: fac_g,fac_gy,fac_gp,fac_gr

!  double precision :: pweight(2)
...
!    double complex :: detX,chdet
    ...
    integer :: r,rid,n0,n1,n2,g,gy,gp,gr,i,rdef,iexp
    logical :: use_pv,use_pv2,use_g,use_gy,use_gp,use_gr
    ...
    double precision :: err_pv(0:rmax),err_pv2(0:rmax),err_g(0:rmax),err_gy(0:rmax),err_gp(0:rmax),err_gr(0:rmax)
...
!     write(*,*) 'CalcCred Xadj=',Xadj
      ...
      Xadj(2,2) = 4d0*mm02*q10 - f(1)*f(1)
      Xadj(1,1) = 4d0*mm02*q20 - f(2)*f(2)
!     Xadj(2,2) = 4d0*mm02*q20 - f(2)*f(2)
!     Xadj(1,1) = 4d0*mm02*q10 - f(1)*f(1)
      Xadj(2,1) = 2d0*mm02*q1q2 - f(1)*f(2)
      Xadj(1,2) = Xadj(2,1)
    end if

!   write(*,*) 'Xadj11 ',4d0*mm02*q10 - f(1)*f(1),mm02,q10,f(1)
!   write(*,*) 'Xadj21 ',2d0*mm02*q1q2 - f(1)*f(2),mm02,q1q2,f(1),f(2)
!   write(*,*) 'Xadj22 ',4d0*mm02*q20 - f(2)*f(2),mm02,q20,f(2)
!   write(*,*) 'Xadjn ',Xadj
!   write(*,*) 'detXn ',detX
!   write(*,*) 'Zadjfn',Zadjf
...
!    write(*,*) 'CalcCred adetX ',adetX,maxZadjf,maxXadj,adetZ
...
!          write(*,*) 'CalcCred w_pv', (maxZadjf/adetZ)**2, abs(mm02)*maxZ/adetZ, maxZ*aZadjff/adetZ**2
!          write(*,*) 'CalcCred err_pv', w_pv**(rdef/2) * err_C0, &
!                        w_pv**(rdef/2-1) * v_pv * z_pv * err_B, err_C0,err_B
...
!      err_req_Cr = acc_req_Cr * abs(C(0,0,0))
      ...
      write(*,*) 'CalcCred Cerr-req=',Cerr-err_req_Cr
...
!    allocate(Cuv_alt(0:rmax,0:rmax,0:rmax))
!    allocate(Cerr_alt(0:rmax))
!    allocate(Cerr2_alt(0:rmax))
!    allocate(Crmethod_alt(0:rmax))
...
! no method works
    if(use_pv.or.use_pv2.or.use_g.or.use_gy.or.use_gp.or.use_gr.eqv..false.) then
...
#ifdef Credtest
    write(*,*) 'iexp=',iexp
    write(*,*) 'facexp=',fac_g,fac_gy,fac_gp,fac_gr
    write(*,*) 'errexp=',err_g_exp,err_gy_exp,err_gp_exp,err_gr_exp,err_req_Cr(rdef)
    write(*,*) 'errexptot=',i
    write(*,*) 'errexptot g1 =',g
    write(*,*) 'errexptot g2 =',err_g(rdef)
    write(*,*) 'errexptot y1 =',gy
    write(*,*) 'errexptot y2 =',err_gy(rdef)
    write(*,*) 'errexptot=',gp,err_gp(rdef)
    write(*,*) 'errexptot=',gr,err_gr(rdef)
    write(*,*) 'errexptot=',i,g,err_g(rdef),gy,err_gy(rdef),gp,err_gp(rdef),gr,err_gr(rdef)
    write(*,*) 'accexptot=',i,g,err_g(rdef)/Ctyp,gy,err_gy(rdef)/Ctyp,gp,err_gp(rdef)/Ctyp,gr,err_gr(rdef)/Ctyp
#endif
...
!        call CopyCimp(C,C_alt,Cerr,Cerr_alt,Crmethod,Crmethod_alt,rmax,rmax)
!        call CopyCimp2(C,C_alt,Cerr,Cerr_alt,Cerr2,Cerr2_alt,Crmethod,Crmethod_alt,rmax,rmax)
...
#ifdef Credtest
    write(*,*) 'CalcCred no optimal method'
    write(*,*) 'err_req=',err_req_Cr(rdef),rdef
    write(*,*) 'err_est=',err_pv(rdef),err_pv2(rdef),err_g(rdef),err_gy(rdef),err_gp(rdef),err_gr(rdef)
#endif
...
#ifdef Credtest
      write(*,*) 'CalcCred: bef final loop expansion depth',r,g,gy,gp,gr
      write(*,*) 'CalcCred: bef final loop err methods',r,err_pv(r),err_pv2(r),err_g(r),err_gy(r),err_gp(r),err_gr(r)
      write(*,*) 'CalcCred: bef final loop acc methods',r,err_pv(r)/Ctyp,err_pv2(r)/Ctyp,    &
             err_g(r)/Ctyp,err_gy(r)/Ctyp,err_gp(r)/Ctyp,err_gr(r)/Ctyp
...
!          call CopyCimp(C,C_alt(0:r,0:r,0:r),Cerr,Cerr_alt(0:r),Crmethod,Crmethod_alt(0:r),rmax,r)
!          call CopyCimp2(C,C_alt(0:r,0:r,0:r),Cerr,Cerr_alt(0:r),Cerr2,Cerr2_alt(0:r),Crmethod,Crmethod_alt(0:r),rmax,r)
...
#ifdef Credtest
      write(*,*) 'CalcCred: final loop err methods',r,err_pv(r),err_pv2(r),err_g(r),err_gy(r),err_gp(r),err_gr(r)
...
!           errorwriteflag)
      ...
      if (CritPointCntC.le.MaxCritPointC) then
...
!             Cerr(rg)=max(Cerr(rg),abs(Cexpgr(0,n1,n2,g))**2/abs(Cexpg(0,n1,n2,g-1)))
...
!    detZ = 4d0*q10*q20-q1q2*q1q2
    ...
    if (abs(detZ).lt.abs(4d0*q10*q20 + q1q2*q1q2)*1d-4) then
      if (abs(q10-q20).lt.abs(q10-q21).and.  &
          abs(q10-q20).lt.abs(q20-q21)) then
        detZ  =  4d0*q10*q21 - (q10-q20+q21)*(q10-q20+q21)
      end if
...
!    write(*,*) 'CalcCgp Cerr2',Cerr2
  ...
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !  subroutine CopyCimp(C,C_alt,Cerr,Cerr_alt,Crmethod,Crmethod_alt,rmax)
  !
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  
  subroutine CopyCimp(C,C_alt,Cerr,Cerr_alt,Crmethod,Crmethod_alt,rmax,r_alt)

    integer,   intent(in) :: rmax,r_alt
    double complex, intent(inout) :: C(0:rmax,0:rmax,0:rmax)
    double precision, intent(inout) :: Cerr(0:rmax)
    integer, intent(inout) :: Crmethod(0:rmax)
    double complex, intent(in) :: C_alt(0:r_alt,0:r_alt,0:r_alt)
    double precision, intent(in) :: Cerr_alt(0:r_alt)
    integer, intent(in) :: Crmethod_alt(0:rmax)

    integer :: r,n1,n0

!    write(*,*) 'CopyCimp: Cerr =',Cerr
!    write(*,*) 'CopyCimp: Cerr_alt =',Cerr_alt
...
!        end forall
   ...
   end subroutine CopyCimp

  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  !  subroutine CopyCimp2(C,C_alt,Cerr,Cerr_alt,Cerr2,Cerr2_alt,Crmethod,Crmethod_alt,rmax)
  !
  !!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!!
  
  subroutine CopyCimp2(C,C_alt,Cerr,Cerr_alt,Cerr2,Cerr2_alt,Crmethod,Crmethod_alt,rmax,r_alt)

    integer,   intent(in) :: rmax,r_alt
    double complex, intent(inout) :: C(0:rmax,0:rmax,0:rmax)
    double precision, intent(inout) :: Cerr(0:rmax),Cerr2(0:rmax)
    integer, intent(inout) :: Crmethod(0:rmax)
    double complex, intent(in) :: C_alt(0:r_alt,0:r_alt,0:r_alt)
    double precision, intent(in) :: Cerr_alt(0:r_alt),Cerr2_alt(0:r_alt)
    integer, intent(in) :: Crmethod_alt(0:rmax)

    integer :: r,n1,n0

!    write(*,*) 'CopyCimp2: Cerr =',Cerr
!    write(*,*) 'CopyCimp2: Cerr_alt =',Cerr_alt
