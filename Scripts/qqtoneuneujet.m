...
If[CalcProcess === "uubar",
	process = {F[3, {1}], -F[3, {1}]} -> {F[11,{1}], F[11,{1}], V[5]};
	name = CalcProcess;
	,
	(*else*)
	If[CalcProcess === "ddbar",
		process = {F[4, {1}], -F[4, {1}]} -> {F[11,{1}], F[11,{1}], V[5]};
		name = CalcProcess;]
	(*else*)
	If[CalcProcess === "ug",
		process = {F[3, {1}], V[5]} -> {F[11,{1}], F[11,{1}], F[3, {1}]};
		name = CalcProcess;]
	(*else*)
	If[CalcProcess === "ubarg",
		process = {-F[3, {1}], V[5]} -> {F[11,{1}], F[11,{1}], -F[3, {1}]};
		name = CalcProcess;]
	(*else*)
	If[CalcProcess === "dg",
		process = {F[4, {1}], V[5]} -> {F[11,{1}], F[11,{1}], F[4, {1}]};
		name = CalcProcess;]
	(*else*)
	If[CalcProcess === "dbarg",
		process = {-F[4, {1}], V[5]} -> {F[11,{1}], F[11,{1}], -F[4, {1}]};
		name = CalcProcess;   
		,
		(*else*)
...
ins = DiagramSelect[ins,(FreeQ[FieldPoints[##],FieldPoint[_][_,_,V[1]]]) &]; (*Diagram does not contain Photon*)

ins = DiagramSelect[ins,FreeQ[LoopFields[##], V[2]] &]; (*Loop does not contain Z*)
ins = DiagramSelect[ins,FreeQ[LoopFields[##], V[1]] &]; (*Loop does not contain Photon*)
ins = DiagramSelect[ins,FreeQ[LoopFields[##], V[3]] &]; (*Loop does not contain W*)

ins = DiagramSelect[ins,FreeQ[FieldPoints[##],FieldPoint[_][_,_,V[3],V[3]]] &]; (*Diagram does not contain 2W*)
